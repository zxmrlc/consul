...
<ConfigEntryReference
  keys={[
    ...
    },
    {
      ...
      enterprise: true,
      description:
        'Specifies the namespaces the config entry will apply to. This must be the namespace' +
        ' the gateway is registered in. If omitted, the namespace will be inherited from [the request](/api/config#ns) or will default to the `default` namespace',
