...
For common errors messages related to Kubernetes, go to [Common errors on Kubernetes](#common-errors-on-kubernetes).
...
this could be because the CNI (Container Networking Interface) does not support the use of `hostPort`. To work around this issue,
...
the need for CNI to support ports mapping.
