...
#### <a name="v-server" href="#v-server">`server`</a> - Values that configure running a Consul server cluster within Kubernetes.
...
#### <a name="v-client" href="#v-client">`client`</a> - Values that configure running a Consul client agent on Kubernetes nodes.
...
#### <a name="v-synccatalog" href="#v-synccatalog">`syncCatalog`</a> - Values that configure running the [service sync](/docs/platform/k8s/service-sync.html) process.
